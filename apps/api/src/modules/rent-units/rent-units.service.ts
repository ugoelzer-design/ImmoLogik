...
export class RentUnitsService {
  ...
  findByObject(objectId: string) {
    return this.prisma.rentUnit.findMany({ where: { objectId }, orderBy: { unitLabel: "asc" } });
  }
  ...
  create(dto: CreateRentUnitDto) {
    return this.prisma.rentUnit.create({ data: { ...dto, istMiete: dto.istMiete ?? 0, zahlungsStatus: dto.zahlungsStatus ?? "Offen" } });
  }
